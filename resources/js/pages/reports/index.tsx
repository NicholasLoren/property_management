...
import type { ReportPropertyRow, ReportSummary } from '@/types/reports';
        ...
        <>
            ...
            <div className="mb-[22px] flex flex-wrap items-end justify-between gap-5">
                <div>
                    ...
                    <p className="mt-1 text-[13px] text-text-secondary">
                        Financial and occupancy performance across your portfolio.
                    </p>
                </div>
            ...
            <div className="mb-5 flex flex-wrap items-end gap-3 rounded-[14px] border border-border-soft bg-card p-4 shadow-sm">
                ...
                <div className="grid gap-1.5">
                    ...
                    <Select
                        ...
                        onValueChange={(value) =>
                            reload({ property_id: value === 'all' ? null : value })
                        }
                    >
            ...
            <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <StatTile
                    ...
                    value={formatCurrency(summary.expected_monthly_rent, currency)}
                />
                <StatTile
                    icon={Wrench}
                            ...
                            breakdown.map((row) => (
                                <tr key={row.property_name}>
                                    ...
                                    <td className="px-4 py-3 text-success">
                                        {formatCurrency(row.rent_collected, currency)}
                                    </td>
                                    <td className="px-4 py-3 text-success">
                                        {formatCurrency(row.other_income, currency)}
                                    </td>
                                    <td className="px-4 py-3 text-destructive">
                                        {formatCurrency(row.total_expense, currency)}
                                    </td>
                                    <td className="px-4 py-3 font-semibold">
                                        {formatCurrency(row.net_income, currency)}
                                    </td>
                                    <td className="px-4 py-3">{row.occupancy_rate}%</td>
                                    <td className="px-4 py-3">{row.maintenance_open}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
